...
public class GuiWSBIMOptionsExtendedSmallList extends GuiScreen{
				...
				if(pos_num < 5){
					...
					if(pos !=null && option.avalableOption){
						if((option.type !=OptionType.INT) && (option.type !=OptionType.FLOAT) && (option.type != OptionType.STRING) && (option.type !=OptionType.GUI)){
							this.buttonList.add(new GuiButtonOption((Option)WSBIM.options.optionsList.get(i), -1, pos.x, pos.y-24, 300, 20, ((Option)WSBIM.options.optionsList.get(i)).guiName));
						}
						else if(option.type == OptionType.GUI){
							this.buttonList.add(new GuiButtonOpenGUI(option, -1, pos.x, pos.y-24, 300, 20, option.guiName));
						}
	...
	public Position getPos(int num){
		switch(num){
			...
			case 4:
				return new Position(this.width / 2 - 155, this.height / 6 + 168 - 6);
