...
public abstract class AbstractMessage<T extends AbstractMessage<T>> implements IMessage, IMessageHandler <T, IMessage>
{
	...
	public final IMessage onMessage(T msg, MessageContext ctx) {
		...
		/**} */else {
			msg.process(WSBIM.proxy.getPlayerEntity(ctx), ctx.side);
		}
		return null;
	}
	...
	private static final <T extends AbstractMessage<T>> void checkThreadAndEnqueue(final AbstractMessage<T> msg, final MessageContext ctx) {
		WorldServer thread = WSBIM.proxy.getThreadFromContext(ctx);
		// pretty much copied straight from vanilla code, see {@link PacketThreadUtil#checkThreadAndEnqueue}
		((Object) thread).addScheduledTask(new Runnable() {
			public void run() {
