...
public class BlockObsidianChest extends BlockContainer {
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state){
        ...
        if (tileentity instanceof TileEntityObsidianChest){
        	InventoryHelper.dropInventoryItems(worldIn, pos, (TileEntityObsidianChest)tileentity);
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ){
		...
		if(owner !=null && !owner.equals(playerIn.getDisplayNameString())){
			...
			playerIn.sendMessage(message1);
				if(worldIn.getPlayerEntityByName(owner) !=null){
					TextComponentString message2 = new TextComponentString(playerIn.getDisplayNameString()+" tried "+"to get"+" into your "+"obsidian "+"che"+"st! "+"@ "+"x="+""+pos.getX()+" y="+""+pos.getY()+" z="+""+pos.getZ());
					message2.getStyle().setColor(TextFormatting.DARK_RED);
					worldIn.getPlayerEntityByName(owner).sendMessage(message2);
				}
			...
			message3.getStyle().setColor(TextFormatting.LIGHT_PURPLE);
			playerIn.sendMessage(message3);
    ...
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand)
    {
        for (int i = 0; i < 3; ++i)
        {
            ...
            if(rand.nextInt(10) == 0)worldIn.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, d0, d1, d2, d3 * 0.25F, d4, d5 * 0.25F);
        }
    }
